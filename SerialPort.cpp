...
void CSerialPort::writeMode(const uint8_t* data, uint8_t length, uint8_t modeFunctionId, uint8_t offset)
{
  uint8_t reply[3U+length+offset];
  uint8_t count = 0U;
  reply[count++] = MMDVM_FRAME_START;
  reply[count++] = 0U;
  for( int i=0; i<offset; i++)
    reply[count++] = 0U;
  reply[count++] = modeFunctionId;

  for (uint8_t i = 0U; i < length; i++)
    reply[count++] = data[i];

  reply[offset + 1U] = count;
  writeInt(1U,reply,count);
}

void CSerialPort::writeModeData(const uint8_t* data, uint8_t length, uint8_t modeDataId)
{
  writeMode(data,length,modeDataId);
}

void CSerialPort::writeModeLost(uint8_t modeLostId)
{
  writeMode(0,0,modeLostId);
}

void CSerialPort::writeModeEOT(uint8_t modeEOTId)
{
  writeMode(0,0,modeEOTId);
}
